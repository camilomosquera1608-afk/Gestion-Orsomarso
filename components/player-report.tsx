...
const last = <T,>(rows: T[]) =>
  rows.length ? rows[rows.length - 1] : undefined;

const validNumber = (value: unknown) =>
  typeof value === "number" && Number.isFinite(value) && value !== 0;
const sum = <T,>(rows: T[], read: (row: T) => unknown) =>
  rows.reduce(
    (acc, row) => acc + (validNumber(read(row)) ? Number(read(row)) : 0),
    0,
  );
...
function MetricGrid({
  ...
  const clean = items
    .map((item) => ({
      ...item,
      rendered: fmt(item.value, item.suffix ?? "", item.decimals),
    }))
...
function KpiGrid({ items }: { items: MetricItem[] }) {
  const clean = filterEmptyMetrics(
...
function ChartSection({
  ...
  return (
    <ReportSection
      icon={BarChart3}
      eyebrow="Evolución"
      title="Evolución diaria por métrica"
    >
    ...
    <div className="player-evaluation-cards">
      {cleanGroups.map((group) => {
        ...
        return (
          <article
            key={group.title}
            className={`player-evaluation-card player-evaluation-${group.tone ?? "blue"}`}
          >
            ...
            {group.note ? (
              <p className="player-evaluation-note">
                {getPdfSafeText(group.note)}
              </p>
            ) : null}
          </article>
        );
...
export function PlayerReportTemplate({
  player,
  category,
  ...
  const playerCategory =
    player.category ??
    ((category === "all" ? "Sub20" : category) as ClubCategory);
  const gpsEnabled = supportsGps(playerCategory);

  const validWellness = wellnessHistory.filter((row) =>
    hasValidValue(row.value),
  );
  const validInternal = internalHistory.filter((row) =>
    hasValidSectionData(row.load, row.rpe, row.duration),
  );
  const validExternal = (gpsEnabled ? externalHistory : []).filter((row) =>
    hasValidSectionData(
      row.min,
      row.acc,
      row.dcc,
      ...
      row.totalDistance,
      row.playerLoad,
      row.highSpeedDistance,
      row.sprintDistance,
    ),
  );
  const validCompetition = competitionHistory.filter((row) =>
    hasValidSectionData(
      row.minutesPlayed,
      row.goals,
      row.assists,
  ...
  const validNutrition = nutritionHistory.filter((row) =>
    hasValidSectionData(
      row.weight,
      row.height,
      row.bodyFat,
      row.skinfoldSum,
      row.imo,
      row.muscleMassPercentage,
      row.diagnosis,
      row.nutritionPlan,
    ),
  );
  const validCmj = cmjHistory.filter((row) =>
    hasValidSectionData(row.value, row.power, row.asymmetry, row.rsi),
  );
  const validFms = fmsHistory.filter((row) =>
    hasValidSectionData(
      row.total,
      row.shoulderMobility,
      row.squat,
      row.legRaise,
      row.hurdleStep,
      row.lunge,
  ...
  const validNeuro = neuromuscularHistory.filter((row) =>
    hasValidSectionData(row.cmj, row.sj, row.reactiveJumps),
  );
  ...
  const wellnessAvg = groupAverage(
    validWellness.map((row) => row.value).filter((value) => value > 0),
  );
  ...
  const hasEvaluations = Boolean(
    latestNutrition || latestCmj || latestFms || latestNeuro,
  );
  const hasCompetition = validCompetition.length > 0;
  const hasMedical =
    player.status !== "Disponible" ||
    medicalSummaryItems.length > 0 ||
    manualMedicalNotes.length > 0;
  const hasGps = gpsEnabled && Boolean(latestExternal);
  const statusTone: "green" | "red" | "amber" =
    player.status === "Disponible"
      ? "green"
  ...
  const coverMetrics = filterEmptyMetrics([
    ...
    {
      label: "Partidos",
      value: validCompetition.length,
      note: fmt(minutes, " min") ?? undefined,
      tone: "dark" as const,
    },
    {
      label: "Carga",
      value: Math.round(internalTotal),
      note: "UA acumulada",
      tone: "blue" as const,
    },
    {
      label: "Última valoración",
      value: latestNutrition?.date ?? latestCmj?.date ?? latestFms?.date ?? "",
      note: "Control",
    ...
    ? [
        {
          ...
          suffix: " min",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.min),
        },
        {
          title: "GPS · Distancia total",
          suffix: " m",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.totalDistance),
        },
        {
          title: "GPS · Player Load",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.playerLoad),
        },
        {
          title: "GPS · Alta velocidad",
          suffix: " m",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.highSpeedDistance),
        },
        {
          title: "GPS · Sprint",
          suffix: " m",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.sprintDistance),
        },
        {
          title: "GPS · ACC",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.acc),
        },
        {
          title: "GPS · DCC",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.dcc),
        },
        {
          title: "GPS · Sprints",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.sprints),
        },
        {
          title: "GPS · RHIE",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.rhie),
        },
        {
          title: "GPS · IMA",
          decimals: 0,
          points: chartPoints(validExternal, (row) => row.ima),
        },
        {
          title: "GPS · RPE",
          decimals: 1,
          points: chartPoints(validExternal, (row) => row.rpe),
        },
      ]
    : [];
  ...
  const charts = [
    {
      title: "Wellness",
      decimals: 1,
      points: chartPoints(validWellness, (row) => row.value),
    },
    {
      title: "Carga interna",
      suffix: " UA",
      decimals: 0,
      points: chartPoints(validInternal, (row) => row.load),
    },
    ...gpsCharts,
    {
      title: "Peso",
      suffix: " kg",
      decimals: 1,
      points: chartPoints(validNutrition, (row) => row.weight),
    },
    {
      title: "IMO",
      decimals: 1,
      points: chartPoints(validNutrition, (row) => row.imo),
    },
    {
      title: "% grasa",
      suffix: "%",
      decimals: 1,
      points: chartPoints(validNutrition, (row) => row.bodyFat),
    },
    {
      title: "% masa muscular",
      suffix: "%",
      decimals: 1,
      points: chartPoints(validNutrition, (row) => row.muscleMassPercentage),
    },
    {
      title: "Minutos competencia",
      suffix: " min",
      decimals: 0,
      points: chartPoints(validCompetition, (row) => row.minutesPlayed),
    },
    {
      title: "CMJ",
      suffix: " cm",
      decimals: 1,
      points: chartPoints(validCmj, (row) => row.value),
    },
    {
      title: "FMS",
      suffix: " pts",
      decimals: 0,
      points: chartPoints(validFms, (row) => row.total),
    },
  ];
    ...
    latestNutrition
      ? {
          ...
          items: [
            ...
            {
              label: "Sumatoria",
              value: latestNutrition.skinfoldSum,
              suffix: " mm",
            },
            {
              label: "% grasa",
              value: latestNutrition.bodyFat,
              suffix: "%",
              decimals: 1,
            },
            {
              label: "% muscular",
              value: latestNutrition.muscleMassPercentage,
              suffix: "%",
              decimals: 1,
            },
    ...
    latestCmj || latestNeuro
      ? {
          ...
          items: [
            {
              label: "CMJ",
              value: latestCmj?.value ?? latestNeuro?.cmj,
              suffix: " cm",
              decimals: 1,
            },
  ...
  return (
    <ReportLayout
      ...
      className={`player-report-document ${className}`}
    >
      <ReportCover
        title="Informe individual del jugador"
        subject={player.name}
      ...
      <section className="pdf-report-hero player-report-hero">
        <div className="player-report-avatar">
          <span>{initials}</span>
        </div>
        <div className="player-report-core">
          ...
          <h2>{player.name}</h2>
          <p>
            {player.position} · {categoryLabel(playerCategory)} · {ageLabel}
          </p>
        ...
        <div className="player-report-meta">
          {latestWellness ? (
            <div>
              ...
              <span>Último wellness</span>
              <strong>{latestWellness.date}</strong>
            </div>
          ) : null}
          {latestInternal || latestExternal ? (
            <div>
              <Activity size={14} />
              <span>Última carga</span>
              <strong>{latestInternal?.date ?? latestExternal?.date}</strong>
            </div>
          ) : null}
        </div>
      </section>
          ...
          items={[
            {
              ...
              value: formatBirthDateForDisplay(player.birthDate),
            },
          ...
          items={[
            {
              icon: HeartPulse,
              label: "Wellness",
              value: wellnessAvg ? wellnessAvg.toFixed(1) : "",
              note: latestWellness?.date,
            ...
            {
              icon: Scale,
              label: "Peso valoración",
              value: latestNutrition ? fmt(latestNutrition.weight, " kg") : "",
              note: latestNutrition?.date,
              tone: "blue",
            },
            {
              icon: Zap,
              label: "CMJ",
              value: latestCmj
                ? fmt(latestCmj.value, " cm")
                : latestNeuro
                  ? fmt(latestNeuro.cmj, " cm")
                  : "",
              note: latestCmj?.date ?? latestNeuro?.date,
              tone: "green",
            },
            {
              icon: ShieldCheck,
              label: "FMS",
              value: latestFms ? fmt(latestFms.total, " pts") : "",
              note: latestFms?.date,
              tone: "blue",
            },
            {
              icon: Zap,
              label: "GPS último",
              value: latestExternal ? fmt(latestExternal.min, " min") : "",
              note: latestExternal?.date,
              tone: "green",
            },
          ]}
        />
      ...
      {!gpsEnabled ? (
        <ReportSection
          icon={ShieldCheck}
          eyebrow="Modelo sin GPS"
          title="Análisis adaptado a carga interna"
          subtitle="Esta categoría no registra GPS; el informe se construye con wellness, RPE, duración, asistencia, competencia, valoraciones y disponibilidad."
        >
          <MetricGrid
            items={[
              {
              ...
              {
                label: "Carga interna total",
                value: Math.round(internalTotal),
                suffix: " UA",
              },
              { label: "Sesiones con RPE", value: validInternal.length },
              { label: "Competencias", value: validCompetition.length },
            ]}
          />
        </ReportSection>
      ) : null}
            ...
            >
              <MetricGrid
                ...
                items={[
                  {
                    label: "Estado",
                    value: player.status !== "Disponible" ? player.status : "",
                  },
                  {
                    label: "Detalle médico",
                    value: medicalSummaryItems.join(" · "),
                  },
                ]}
              />
              {manualMedicalNotes ? (
                <p className="pdf-manual-note">
                  {getPdfSafeText(manualMedicalNotes)}
                </p>
              ) : null}
            </ReportSection>
          ) : null}
      ...
      {hasEvaluations ? (
        <ReportSection
          ...
          subtitle="Lectura limpia de nutrición, salto, neuromuscular y FMS."
        >
        ...
        <ReportSection icon={Zap} eyebrow="GPS" title="Carga externa integrada">
          <MetricGrid
            items={[
              ...
              {
                label: "Distancia",
                value: latestExternal?.totalDistance,
                suffix: " m",
              },
              { label: "Player Load", value: latestExternal?.playerLoad },
              {
                label: "HSR",
                value: latestExternal?.highSpeedDistance,
                suffix: " m",
              },
              {
                label: "Sprint",
                value: latestExternal?.sprintDistance,
                suffix: " m",
              },
