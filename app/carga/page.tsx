  ...
  return (
    <div className="grid load-center-page">
      ...
      <div className="grid grid-4">
        ...
        <KpiCard label="MIN acumulados" value={String(center.totals.minutes)} tone="green" icon={<Timer size={18} />} trend="Volumen" />
        <KpiCard label="RPE promedio" value={center.totals.avgRpe.toFixed(1)} tone="amber" icon={<TrendingUp size={18} />} trend="RPE" />
      ...
      <div className="grid grid-2">
        <div className="card">
          ...
          ) : <EmptyState title="Sin carga registrada" text="Sin registros de carga." />}
        </div>
        <div className="card">
          ...
          <div className="load-risk-grid">
            ...
            <div className="load-risk-panel ui-tone-blue">
              ...
              <span>{center.lowExposure.length} jugador(es)</span>
            </div>
          </div>
          <div className="load-player-list">
            {[...center.highLoad, ...center.lowExposure].slice(0, 10).map((row) => (
            ...
            {!center.highLoad.length && !center.lowExposure.length ? <EmptyState icon="check" title="Sin alertas" text="" /> : null}
          </div>
        </div>
      </div>

      <div className="card">
        <SectionHeader eyebrow="Plantel" title="Ranking de carga" />
        <div className="professional-table-wrap">
          ...
          {!center.rows.length ? <EmptyState title="Sin jugadores" text="Ajusta los filtros." /> : null}
        </div>
      </div>
    </div>
  );
