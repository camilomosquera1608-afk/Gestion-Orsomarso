...
import { ClubCategory, SessionParticipation, TrainingSessionType } from '@/lib/types';
...
type RowState = { selected: boolean; participation: SessionParticipation; min: number; rpe: number; acc: number; dcc: number; sprints: number; rhie: number; ima: number; };
...
export default function SesionEntrenamientoPage() {
  ...
  const sessionPlayers = useMemo(() => data.players.filter((player) => player.category === activeCategory && (filters.position === 'all' || player.position === filters.position) && (filters.status === 'all' || player.status === filters.status)), [data.players, filters.position, filters.status, activeCategory]);
  const existingRecords = useMemo(() => data.externalLoads.filter((record) => {
    const player = data.players.find((p) => p.id === record.playerId);
    return !!player && player.category === activeCategory && (record.microcycleId ?? filters.microcycleId) === filters.microcycleId && (record.sessionNumber ?? 1) === filters.sessionNumber && record.date === filters.date;
  }).sort((a, b) => a.playerId.localeCompare(b.playerId)), [data.externalLoads, data.players, filters.microcycleId, filters.sessionNumber, filters.date, activeCategory]);
  ...
  useEffect(() => {
    ...
    sessionPlayers.forEach((player) => {
      ...
      next[player.id] = { selected: !!existing, participation: existing?.participation ?? 'Completa', min: existing?.min ?? 0, rpe: existing?.rpe ?? 0, acc: existing?.acc ?? 0, dcc: existing?.dcc ?? 0, sprints: existing?.sprints ?? 0, rhie: existing?.rhie ?? 0, ima: existing?.ima ?? 0 };
    });
    setRowStates(next);
  ...
  const rows = sessionPlayers.map((player) => ({ player, ...(rowStates[player.id] ?? { selected: false, participation: 'Completa', min: 0, rpe: 0, acc: 0, dcc: 0, sprints: 0, rhie: 0, ima: 0 }) }));
  const selectedRows = rows.filter((row) => row.selected);

  const updateRow = (playerId: string, patch: Partial<RowState>) => setRowStates((prev) => ({ ...prev, [playerId]: { ...(prev[playerId] ?? { selected: false, participation: 'Completa', min: 0, rpe: 0, acc: 0, dcc: 0, sprints: 0, rhie: 0, ima: 0 }), ...patch } }));

  const saveSession = () => {
    upsertTrainingSessionSummary({ id: summaryRecord?.id ?? crypto.randomUUID(), date: filters.date, microcycleId: filters.microcycleId, sessionNumber: filters.sessionNumber, sessionType, objective, observation });
    ...
    selectedRows.forEach((row) => {
      ...
      upsertInternalLoad({ id: crypto.randomUUID(), playerId: row.player.id, date: filters.date, rpe: row.rpe, duration: row.min, microcycleId: filters.microcycleId, sessionNumber: filters.sessionNumber, category: activeCategory });
    });
        ...
        <div className="btn-row" style={{ justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div className="btn-row">
            <button type="button" className="btn secondary" onClick={() => downloadCsv(`sesion-${activeCategory}.csv`, selectedRows.map((r) => ({
              fecha: filters.date, jugador: r.player.name, categoria: activeCategory, participacion: r.participation, minutos: r.min, rpe: r.rpe, acc: youthSimple ? '' : r.acc, dcc: youthSimple ? '' : r.dcc, sprints: youthSimple ? '' : r.sprints, rhie: youthSimple ? '' : r.rhie, ima: youthSimple ? '' : r.ima
            })))}>Exportar CSV</button>
          ...
          {rows.map((row) => (
            <div key={row.player.id} className="session-player-card clean-mobile-card">
              <div className="session-player-header">
                <div><strong>{row.player.name}</strong><div className="muted-line">{row.player.position} · {row.player.category}</div></div>
                <div className="btn-row">
