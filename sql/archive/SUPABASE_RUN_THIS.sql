...
-- Copia incluida para ejecutar todo en un solo archivo.
-- ============================================================
-- Orsomarso Performance App
-- v121 - Fecha de nacimiento estable e informe individual unificado
-- Seguro: no borra datos. Ejecutar una vez en Supabase SQL Editor.
...
-- ============================================================
-- Orsomarso Performance App
-- V131 - Fix visibilidad de mapa corporal en staff/Vercel
-- Seguro: no borra datos. Ejecutar en Supabase SQL Editor.
-- Problema corregido: la app usa sesión propia de staff y consulta Supabase con rol anon,
-- por eso la pantalla /disponibilidad podía no leer body_map_reports aunque existieran datos.
...
alter table public.body_map_reports add column if not exists legacy_id text;
alter table public.body_map_reports add column if not exists player_legacy_id text;
alter table public.body_map_reports add column if not exists category text;
alter table public.body_map_reports add column if not exists source text not null default 'Jugador';
alter table public.body_map_reports add column if not exists type text not null default 'Molestia';
alter table public.body_map_reports add column if not exists region text not null default 'Otro';
alter table public.body_map_reports add column if not exists side text not null default 'Central';
alter table public.body_map_reports add column if not exists intensity numeric not null default 0;
alter table public.body_map_reports add column if not exists limitation boolean not null default false;
...
alter table public.body_map_reports add column if not exists mechanism text;
alter table public.body_map_reports add column if not exists notes text;
alter table public.body_map_reports add column if not exists action text;
alter table public.body_map_reports add column if not exists restriction text;
alter table public.body_map_reports add column if not exists load_allowed_pct numeric;
alter table public.body_map_reports add column if not exists status text not null default 'Abierto';
alter table public.body_map_reports add column if not exists created_at timestamptz not null default now();
alter table public.body_map_reports add column if not exists updated_at timestamptz not null default now();

create unique index if not exists ux_body_map_reports_legacy_id_not_null
  on public.body_map_reports(legacy_id)
  where legacy_id is not null;
...
drop trigger if exists trg_body_map_reports_updated_at on public.body_map_reports;
create trigger trg_body_map_reports_updated_at
...
-- Lectura para la app desplegada en Vercel. La app maneja login/rol propio y filtra por categoria.
...
drop policy if exists body_map_app_read on public.body_map_reports;
create policy body_map_app_read
on public.body_map_reports
for select
to anon, authenticated
using (true);

-- Insercion desde wellness publico: solo reportes de jugador, categorias validas y fechas recientes.
...
-- Actualizacion anon limitada a reportes del jugador; el staff puede actualizar por la app autenticada/rol propio.
...
-- ============================================================
-- V132 - Sincronizacion de datos compartidos
-- ============================================================
